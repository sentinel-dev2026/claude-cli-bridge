...
もともとは、**OpenClaw から直接 Anthropics 系のサブスク利用をつなぐ運用がしづらくなった**ため、
**OpenClaw 自体は別の LLM で動かしつつ、必要な場面だけ Claude Code CLI をサブスク枠で使う**ために作られました。

つまりこのプラグインは、

- 普段の OpenClaw は別モデルで動かす
- でも Claude に回したい重い作業・深い推論・コードレビューは Claude Code CLI に任せる
- その橋渡しを OpenClaw の内部ツールとして行う
...
- **Claude Code CLI で事前に一度 OAuth ログインを済ませてあること**

### 4. Claude Code CLI で事前認証しておく

このプラグインは OpenClaw から `claude` CLI を呼び出すだけなので、
**Claude Code CLI 側の認証が未完了だと動きません**。

そのため、最初に人の手で一度 OAuth ログインを済ませておく必要があります。

イメージとしては:

1. ターミナルで `claude` を直接起動
2. 表示される案内に従って OAuth ログイン
3. Claude CLI が通常利用できる状態にする
4. その後に OpenClaw からこのプラグイン経由で呼び出す

要するに、**このプラグインは認証そのものは代行せず、認証済みの Claude CLI を利用する**構成です。
...
- 利用前に **Claude Code CLI の OAuth 認証を人の手で一度済ませておく必要があります**
